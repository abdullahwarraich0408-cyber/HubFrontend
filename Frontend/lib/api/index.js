...
export const ordersApi = {
  getAll: () => api.get("/orders", { auth: "customer" }),
  getAdminAll: () => api.get("/admin/orders"),
  getById: (id) => api.get(`/orders/${id}`, { auth: "customer" }),
  create: (data) => api.post("/orders", data, { auth: "customer" }),
...
export const prescriptionOrdersApi = {
  create: (formData) => api.post("/prescription-orders", formData),
  getAll: () => api.get("/prescription-orders"),
  getById: (id) => api.get(`/prescription-orders/${id}`),
  confirm: (id, confirmed = true) =>
    api.post(`/prescription-orders/${id}/confirm`, { confirmed }),
  retry: (id) => api.post(`/prescription-orders/${id}/retry`),
  getVendorOrders: () => api.get("/prescription-orders/vendor"),
  getVendorHistory: () => api.get("/prescription-orders/vendor/history"),
  getAdminAll: () => api.get("/admin/prescription-orders"),
...
export const doctorsApi = {
  ...
  getSlots: (id, date, params = {}) => {
    const search = new URLSearchParams({ ...(date ? { date } : {}), ...params });
    const query = search.toString();
    return api.get(query ? `/doctors/${id}/slots?${query}` : `/doctors/${id}/slots`);
  },
  getPracticeLocations: (id) => api.get(`/doctors/${id}/practice-locations`),
  ...
  selectConsultationMode: (id, mode) =>
    api.patch(`/doctors/appointments/${id}/mode`, { mode }, { auth: "customer" }),
...
export const hospitalsApi = {
  getAll: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(query ? `/hospitals?${query}` : "/hospitals");
  },
  getById: (id) => api.get(`/hospitals/${id}`),
  getDoctors: (id, params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(query ? `/hospitals/${id}/doctors?${query}` : `/hospitals/${id}/doctors`);
  },
};
...
export const labTestsApi = {
  ...
  getLabs: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return api.get(query ? `/lab-tests/labs?${query}` : "/lab-tests/labs");
  },
  getLabById: (id) => api.get(`/lab-tests/labs/${id}`),
  ...
  createOrder: (data) => api.post("/lab-tests/orders", data),
