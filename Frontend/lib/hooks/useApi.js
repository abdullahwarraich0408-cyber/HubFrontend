...
import { mapOrdersToFrontend, mapOrderToFrontend, mergeAllOrders, parseOrderRef, mapMedicineOrderToFrontend, mapDoctorAppointmentToOrder, mapLabBookingToOrder, mapPrescriptionOrderToOrder } from "@/lib/mappers/order";
import { mapHospitalsToFrontend, mapHospitalToFrontend } from "@/lib/mappers/hospital";
...
import {
  ...
  mapPrescriptionOrdersToFrontend,
} from "@/lib/mappers/prescriptionOrder";
...
export function useAllOrders(options = {}) {
  return useQuery({
    queryKey: ["all-orders"],
    queryFn: async () => {
      const [ordersResult, appointmentsResult, bookingsResult, prescriptionResult] = await Promise.all([
        ordersApi.getAll().catch(() => ({ orders: [] })),
        doctorsApi.getMyAppointments({ auth: "customer" }).catch(() => ({ appointments: [] })),
        labTestsApi.getMyBookings({ auth: "customer" }).catch(() => ({ bookings: [] })),
...
export function useOrderDetail(orderRef, options = {}) {
  const { type, id } = parseOrderRef(orderRef);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (type === "doctor") {
        const data = await doctorsApi.getMyAppointment(id, { auth: "customer" });
        return mapDoctorAppointmentToOrder(data.appointment);
      }

      if (type === "lab") {
        const data = await labTestsApi.getMyBookings({ auth: "customer" });
        const booking = (data.bookings || []).find((item) => item.id === id);
        if (!booking) throw new Error("Lab booking not found");
        return mapLabBookingToOrder(booking);
      }

      if (type === "prescription") {
        const data = await prescriptionOrdersApi.getById(id);
        return mapPrescriptionOrderToOrder(data.order);
      }

      const data = await ordersApi.getById(id);
      return mapMedicineOrderToFrontend(data.order);
    },
    ...options,
  });
}
...
export function useOrder(id, options = {}) {
  return useOrderDetail(id, options);
}
...
export function useUpdateProfileData() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (profile_data) => usersApi.updateProfile({ profile_data }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["user-profile"] }),
  });
}
...
export function useCreatePrescriptionOrder() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (formData) => prescriptionOrdersApi.create(formData),
    onSuccess: () => {
...
export function usePrescriptionOrders(options = {}) {
  return useQuery({
...
export function usePrescriptionOrder(id, options = {}) {
  return useQuery({
    queryKey: ["prescription-orders", id],
    enabled: Boolean(id),
    queryFn: async () => {
      const data = await prescriptionOrdersApi.getById(id);
      return mapPrescriptionOrderToFrontend(data.order);
    },
    refetchInterval: (query) => {
      const status = query.state.data?.status;
      if (["awaiting_accept", "finding_vendor", "customer_review"].includes(status)) {
        return 3000;
      }
      return false;
    },
...
export function useConfirmPrescriptionOrder() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, confirmed }) => prescriptionOrdersApi.confirm(id, confirmed),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: ["prescription-orders"] });
      queryClient.invalidateQueries({ queryKey: ["prescription-orders", id] });
      queryClient.invalidateQueries({ queryKey: ["all-orders"] });
    },
  });
}

export function useRetryPrescriptionOrder() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id) => prescriptionOrdersApi.retry(id),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: ["prescription-orders"] });
      queryClient.invalidateQueries({ queryKey: ["prescription-orders", id] });
    },
...
export function useHospital(id, options = {}) {
  return useQuery({
...
export function useHospitalDoctors(id, params = {}, options = {}) {
  return useQuery({
...
export function useDoctorSlots(id, date, slotParams = {}, options = {}) {
  return useQuery({
...
export function useSelectConsultationMode() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, mode }) => doctorsApi.selectConsultationMode(id, mode),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["doctor-appointments"] }),
  });
}
...
export function useLabs(params = {}, options = {}) {
  return useQuery({
...
export function useLab(id, options = {}) {
  return useQuery({
    queryKey: ["labs", id],
    enabled: Boolean(id),
    queryFn: async () => {
...
export function useCreateLabOrder() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload) => labTestsApi.createOrder(payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["lab-test-bookings"] });
...
export function useLabReports(options = {}) {
  return useQuery({
    queryKey: ["lab-reports"],
    queryFn: async () => {
      const data = await labTestsApi.getMyReports();
      return (data.reports || []).map(mapLabTestBookingToFrontend);
    },
    ...options,
  });
}
...
export function useCancelLabBooking() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id) => labTestsApi.cancelBooking(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["lab-test-bookings"] });
      queryClient.invalidateQueries({ queryKey: ["all-orders"] });
    },
